                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            @forelse ($categories as $category)
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $category->active ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300' }}">
                                        ...
                                        <div class="flex items-center space-x-2">
                                            @if($category->active)
                                                ...
                                                <form action="{{ route('categories.destroy', $category) }}" method="POST" class="inline"
                                                    onsubmit="return confirm('Tem certeza que deseja desativar esta categoria?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                            ...
                                            @else
                                                <form action="{{ route('categories.activate', $category) }}" method="POST" class="inline">
                                                    @csrf
                                                    <button type="submit"
                                ...
                                <tr>
                                    <td colspan="3" class="px-6 py-12 text-center">
                                        <div class="text-gray-500 dark:text-gray-400">
                                            ...
                                            <p class="mt-1">Comece criando sua primeira categoria</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
